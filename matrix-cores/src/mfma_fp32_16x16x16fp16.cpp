...
constexpr int N = 16;
constexpr int K = 16;
constexpr unsigned int compute_repetitions = 1; // Global constant for repetitions
...
__global__ void sgemm_16x16x16(const float16_t* A, const float16_t* B, float* D)
{
  ...
  floatx4 d = {0}; // Zero out AccVGPRs
  float16x4 a, b;

  for (int i = 0; i < 4; ++i) {
    int a_idx = threadIdx.x * LDA + i + threadIdx.y * 4;
    int b_idx = threadIdx.x + i * LDB + threadIdx.y * LDB * 4;
...
int main()
{
  if (!gpuArchCheck("gfx90a") && !gpuArchCheck("gfx908")) {
    std::cout << "mfma_f32_16x16x16f16 instruction only available on gfx908 or later." << std::endl;
    exit(-1);
  }
  ...
  std::vector<float16_t> A_h(A_size), B_h(B_size);
  ...
  HIP_CHECK(hipMemcpy(D_h.data(), D_d, D_size * sizeof(float), hipMemcpyHostToDevice));
